  ...
  \***********************************/
        ...
        async function trySubmitForm() {
            const menuItemSchema = joi__WEBPACK_IMPORTED_MODULE_1__.object({
                name: joi__WEBPACK_IMPORTED_MODULE_1__.string()
                    ...
                    .max(30)
                    .required(),
                price: joi__WEBPACK_IMPORTED_MODULE_1__.number()
                    .precision(2)
                    .required(),
                courses: joi__WEBPACK_IMPORTED_MODULE_1__.string()
                    .required(),
                vegetarian: joi__WEBPACK_IMPORTED_MODULE_1__.boolean()
                    .required(),
                spiceLevel: joi__WEBPACK_IMPORTED_MODULE_1__.number()
                    .integer()
                    .min(0)
                    .max(10)
                    .required()
        ...
        async function fillMenuItemsTable() {
            try {
                const response = await fetch('/api/menu-items', {
                    headers: {
                        'Accept': 'application/json', [_util_csrf_js__WEBPACK_IMPORTED_MODULE_2__.header]: _util_csrf_js__WEBPACK_IMPORTED_MODULE_2__.token
                    }
                })
                if (response.ok) {
        ...
        async function handleDeleteMenuItem(event) {
            ...
            const response = await fetch(`/api/menu-items/${menuItemId}`, {
                method: 'DELETE', headers: {
                    [_util_csrf_js__WEBPACK_IMPORTED_MODULE_2__.header]: _util_csrf_js__WEBPACK_IMPORTED_MODULE_2__.token
                }
            })
        ...
        async function addNewMenuItem() {
            ...
            try {
                const response = await fetch('/api/menu-items', {
                    method: 'POST', headers: {
                        'Accept': 'application/json', 'Content-Type': 'application/json', [_util_csrf_js__WEBPACK_IMPORTED_MODULE_2__.header]: _util_csrf_js__WEBPACK_IMPORTED_MODULE_2__.token
                    }, body: JSON.stringify({
                        name: nameInput.value,
                        price: priceInput.value,
