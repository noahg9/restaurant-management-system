...
public class ChefController extends BaseController {
    ...
    public ModelAndView allChefs(HttpSession session, Model model, ModelAndView mav) {
        ...
        mav.addObject("all_chefs", chefService.getAllChefs().stream().map(chef -> new ChefViewModel(chef.getId(), chef.getFirstName(), chef.getLastName(), chef.getDateOfBirth(), chef.getUsername(), chef.getPassword(), chef.getRole().getName(), false)).toList());
    ...
    public String updateChef(@Valid ChefViewModel chefViewModel, BindingResult bindingResult, @AuthenticationPrincipal CustomUserDetails user, HttpServletRequest request) {
        ...
        if ((user.getChefId() == chefViewModel.getId() || request.isUserInRole(HEAD_CHEF.getCode())) && (!bindingResult.hasErrors())) {
            chefService.changeChef(chefViewModel.getId(), chefViewModel.getFirstName(), chefViewModel.getLastName(), chefViewModel.getDateOfBirth(), chefViewModel.getUsername(), chefViewModel.getPassword(), chefViewModel.getRoleName());
        }
